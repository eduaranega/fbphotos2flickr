...
import com.fbphotos2flickr.app.FlickrGetOAuthTokenTask;
import com.fbphotos2flickr.app.FlickrOAuthTask;
import com.fbphotos2flickr.app.FlickrUploadPhotoTask;
...
public class FlickrjActivity extends Activity {
	...
	Runnable init = new Runnable() {
		...
		public void run() {
			...
			if (oauth == null || oauth.getUser() == null) {
				FlickrOAuthTask task = new FlickrOAuthTask(getContext());
				task.execute();
			} else {
				load(oauth);
			}
	...
	private void load(OAuth oauth) {
		if (oauth != null) {
			...
			FlickrUploadPhotoTask taskUpload = new FlickrUploadPhotoTask(this, new File(
					path));
			taskUpload.setOnUploadDone(new FlickrUploadPhotoTask.onUploadDone() {
		...
		if (CALLBACK_SCHEME.equals(scheme)
			...
			if (data != null && data.length == 2) {
				...
				if (oauth != null && oauth.getToken() != null
					...
					FlickrGetOAuthTokenTask task = new FlickrGetOAuthTokenTask(this);
